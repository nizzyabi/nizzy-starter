      ...
      <section className="w-full flex items-center justify-center">
        <div className="container px-4 md:px-6">
          ...
          <div className="flex items-center justify-center">
            {tiers.map((tier) => (
              <div key={tier.name} className={`relative flex flex-col p-6 shadow-lg rounded-lg justify-between border bg-secondary w-full lg:w-[550px] mx-auto ${tier.popular ? 'border-purple-500' : 'border-primary/20 bg-secondary'}`}>
                {tier.popular && (
                ...
                {/* Button */}
                <div className="mt-6">
                  <Button onClick={onClick} className={`w-full flex ${tier.popular ? 'text-primary/70 bg-gradient-to-r from-pink-500 to-purple-500 text-white' : ''}`}>
                    {tier.cta}
